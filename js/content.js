...
(function () {
	...
	const HOVER_BG = 'rgba(74, 144, 217, 0.06)';
	const HOVER_DESELECT_OUTLINE = '2px auto #4A90D9FF';
	const HOVER_DESELECT_BG = 'rgba(74, 144, 217, 0.12)';
	const TEXT_LINK_SELECTED_BG = 'rgba(74, 144, 217, 0.25)';
	const TEXT_LINK_HOVER_BG = 'rgba(74, 144, 217, 0.12)';
...
(function () {
	...
	function initGestures() {
		...
		{
			eventManager.add(isWheelGestureEnabled, window, 'wheel', handleWheelGesture, { capture: true, passive: false });
		}
			...
			if (LOCAL_ACTIONS.has(action)) {
				...
				switch (action) {
					...
					case 'menuShowTabs': {
						...
						if (result?.success) {
							const getIcon = (tab) => tab.favIconUrl?.startsWith('data:') ? tab.favIconUrl : '';
					...
					case 'menuRecentlyClosed': {
						...
						if (result?.success) {
							...
							const items = result.tabs.map(tab => ({
								label: tab.title ?? tab.url,
								icon: tab.url ? getIcon(tab) : '',
								time: td !== 'none' && tab.lastModified ? (tab.lastModified) : undefined,
								onClick: () => {
